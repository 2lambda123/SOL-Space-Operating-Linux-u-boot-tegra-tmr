...
void main_loop(void) {
  ...
  // There are four files and four hashes per partition (no rootfs at this
  // stage), and each is placed one after the other. Info file starts at zero
  // offset (mem addr 0xa5000000) and is 1 block long (512 bytes). Hashes are
  // also one block. The remaining files (excluding rootfs) have their offsets
  // and sizes defined in the "BLOB Stuff" section of the machine configuration
  // in Yocto
  ...
  if (YOCTO_SOL_ENABLE_TMR) {
    ...
    if (tmr_blob(
            part_offset[0] + file_offset[0], part_offset[0] + file_offset[1],
            part_offset[1] + file_offset[0], part_offset[1] + file_offset[1],
            part_offset[2] + file_offset[0], part_offset[2] + file_offset[1],
            sizes[0], outputs[0])) {
      safe[2] = 'x';
    }
    ...
    memcpy((void *)outputs[0], (void *)part_offset[0] + file_offset[0],
           sizes[0]); // Info file
    memcpy((void *)outputs[1], (void *)part_offset[0] + file_offset[2],
           sizes[1]); // Image File
    memcpy((void *)outputs[2], (void *)part_offset[0] + file_offset[4],
           sizes[2]); // DTB File
    memcpy((void *)outputs[3], (void *)part_offset[0] + file_offset[6],
           sizes[3]); // Initrd File
  }
  ...
  cli_simple_process_macros(
      "${cbootargs} root=/dev/ram0 rw rootwait ${bootargs}", bootargs);
  ...
  if (!abortboot(5)) {
    ...
    char *argv[4] = {"booti", image_loc, initrd_loc, dtb_loc};
